...
def detect_captcha_type(html: str, images: List[str]) -> str:
    ...
    # CRITICAL FIX: Check IMAGE COUNT FIRST!
    # ROTATE always has 2 images (inner + outer)
    # SLIDER only has 1 image (puzzle)
    # Image count is MORE RELIABLE than keywords!
    
    # TYPE 1: ROTATE - Check FIRST based on image count!
    # - Có 2 ảnh (background + foreground) - MOST RELIABLE INDICATOR
    # - Không có question
    # TikTok rotate captcha ALWAYS has exactly 2 images
    if num_images == 2:
        question = extract_question(html)
        if not question:
            print("[DEBUG] ✅ Detected as ROTATE (2 images + no question) - RELIABLE!")
            return 'rotate'
    
    # TYPE 2: SLIDER - Check AFTER rotate
    # - Có slider button (secsdk-captcha-drag-icon)
    # - Text "Drag the slider"
    # - Usually has 1 image (or 1 large + small icon)
    # Only check slider keyword if NOT 2 images (to avoid false positive)
