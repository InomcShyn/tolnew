...
class ModernChromeProfileManager:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Chrome Profile Manager - Modern Interface")
    ...
    def create_auto_tabbar(self):
        ...
        # Logo compact
        logo_label = ttk.Label(top_row, text="🚀", 
    ...
    def show_export_tab(self):
        ...
        ttk.Button(button_frame, text="🚀 Bắt đầu Export", 
                  style='Modern.TButton',
    ...
    def show_import_tab(self):
        ...
        ttk.Button(button_frame, text="🚀 Bắt đầu Import", 
                  style='Modern.TButton',
...
• Chrome Web Store: https://chromewebstore.google.com/detail/proxy-switchyomega-3-zero/pfnededegaaopdmhkdmcofjmoldfiped
        ...
        ttk.Button(row1, text="🚀 Install for All Profiles", 
                  style='Modern.TButton',
        ...
        ttk.Button(row3, text="🚀 Auto-Install Startup", 
                  style='Modern.TButton',
        ...
        ttk.Button(row4, text="🚀 Auto-Install Startup", 
                  style='Modern.TButton',
                  command=self.auto_install_extension_startup).pack(side=tk.LEFT, padx=(0, 10))
    ...
    def show_general_extensions_installer(self, select_all: bool = False):
        ...
        ttk.Button(actions, text="🚀 Start Install", command=start_install).pack(side=tk.LEFT)
        ...
        ttk.Button(config_buttons_frame, text="🚀 Configure All Profiles", 
                  style='Modern.TButton',
                  command=self.configure_proxy_all).pack(side=tk.LEFT, padx=(0, 10))
    ...
    def show_email_config_tab(self):
        ...
        try:
            ...
            self.proxy_status.insert(tk.END, f"🚀 Configuring proxy for profile: {profile_name}\n")
    ...
    def show_email_config_tab(self):
        ...
        # Tiêu đề
        title_label = ttk.Label(self.content_frame, text="🚀 Auto TikTok 2FA Configuration", 
                               font=("Segoe UI", 16, "bold"))
        title_label.pack(pady=(0, 20))
        ...
        guide_content = """🚀 Auto TikTok 2FA - Hướng dẫn sử dụng:
...
- Hệ thống tự động chọn phương pháp tốt nhất"""
    ...
    def setup_context_menu(self):
        ...
        self.context_menu.add_command(label="▶️ Khởi động (Hiển thị)", 
                                     command=lambda: self.launch_profile(False))
        self.context_menu.add_command(label="▶️ Khởi động (Ẩn)", 
                                     command=lambda: self.launch_profile(True))
        self.context_menu.add_command(label="▶️ Khởi động (Mặc định)", 
            ...
            for item in self.tree.get_children():
                if self.tree.item(item, 'text') == profile_name:
                    ...
                    if len(current_values) >= 3:
                        ...
                        current_values[2] = "Dừng" if "Running" in status else "Khởi động"  # Action column
...
Bạn có muốn tiếp tục?"""
    ...
    def update_profile_combos(self):
        ...
        try:
            ...
            if hasattr(self, 'import_profile_combo'):
    ...
    def show_bulk_create_dialog(self):
        ...
        # Profile gốc
        ttk.Label(info_frame, text="Profile gốc:", font=("Segoe UI", 10, "bold")).pack(anchor=tk.W, pady=(0, 5))
        bulk_source_var = tk.StringVar(value="Default")
        bulk_source_combo = ttk.Combobox(info_frame, textvariable=bulk_source_var, width=47, font=("Segoe UI", 10))
        
        # Lấy danh sách profiles hiện có
        try:
            existing_profiles = self.manager.get_all_profiles()
            bulk_source_combo['values'] = ["Default"] + existing_profiles
        except:
            bulk_source_combo['values'] = ["Default", "Profile 1", "Profile 2", "Profile 3"]
        
        bulk_source_combo.pack(fill=tk.X, pady=(0, 10))
        ...
        # Prefix tên
        ttk.Label(quantity_frame, text="Prefix:", font=("Segoe UI", 9, "bold")).pack(side=tk.LEFT, padx=(0, 5))
        bulk_prefix_var = tk.StringVar(value="Profile")
        bulk_prefix_entry = ttk.Entry(quantity_frame, textvariable=bulk_prefix_var, width=15, font=("Segoe UI", 9))
        bulk_prefix_entry.pack(side=tk.LEFT, padx=(0, 15))
        ...
        def update_bulk_preview():
            try:
                ...
                prefix = bulk_prefix_var.get().strip() or "Profile"
                start = int(bulk_start_var.get())
                
                names = []
                for i in range(quantity):
                    names.append(f"{prefix}_{start + i}")
        ...
        bulk_prefix_var.trace('w', lambda *args: update_bulk_preview())
        bulk_start_var.trace('w', lambda *args: update_bulk_preview())
        
        # Stealth Configuration cho bulk
        bulk_stealth_frame = ttk.LabelFrame(main_frame, text="🥷 Cấu hình Stealth", padding="10")
        bulk_stealth_frame.pack(fill=tk.X, pady=(0, 10))
        
        # Checkbox sử dụng stealth
        bulk_use_stealth_var = tk.BooleanVar(value=False)
        bulk_stealth_check = ttk.Checkbutton(bulk_stealth_frame, text="Áp dụng cấu hình Stealth cho tất cả", 
                                           variable=bulk_use_stealth_var, command=lambda: toggle_bulk_stealth_options())
        bulk_stealth_check.pack(anchor=tk.W, pady=(0, 8))
        
        # Frame cho stealth options
        bulk_stealth_options_frame = ttk.Frame(bulk_stealth_frame)
        
        # Stealth config selection
        bulk_config_select_frame = ttk.Frame(bulk_stealth_options_frame)
        bulk_config_select_frame.pack(fill=tk.X, pady=(0, 8))
        
        ttk.Label(bulk_config_select_frame, text="Cấu hình:", font=("Segoe UI", 8, "bold")).pack(side=tk.LEFT, padx=(0, 5))
        bulk_stealth_config_var = tk.StringVar(value="Default")
        bulk_stealth_config_combo = ttk.Combobox(bulk_config_select_frame, textvariable=bulk_stealth_config_var, width=20)
        
        # Load stealth configs
        try:
            stealth_configs = self.manager.get_stealth_configs_list()
            bulk_stealth_config_combo['values'] = ["Default"] + stealth_configs
        except:
            bulk_stealth_config_combo['values'] = ["Default"]
        
        bulk_stealth_config_combo.pack(side=tk.LEFT, padx=(0, 8))
        
        # Button để tạo stealth config mới
        ttk.Button(bulk_config_select_frame, text="⚙️ Tạo mới", 
                  command=lambda: self.show_stealth_config_dialog()).pack(side=tk.LEFT)
        
        # Quick stealth settings
        bulk_quick_frame = ttk.LabelFrame(bulk_stealth_options_frame, text="⚡ Cài đặt nhanh", padding="8")
        bulk_quick_frame.pack(fill=tk.X, pady=(0, 8))
        
        # CPU và Memory
        bulk_resource_frame = ttk.Frame(bulk_quick_frame)
        bulk_resource_frame.pack(fill=tk.X, pady=(0, 3))
        
        # CPU Cores
        bulk_cpu_frame = ttk.Frame(bulk_resource_frame)
        bulk_cpu_frame.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 8))
        
        ttk.Label(bulk_cpu_frame, text="CPU:", font=("Segoe UI", 8, "bold")).pack(anchor=tk.W)
        bulk_create_cpu_cores = tk.StringVar(value="2")
        bulk_cpu_spinbox = ttk.Spinbox(bulk_cpu_frame, from_=1, to=8, textvariable=bulk_create_cpu_cores, width=6)
        bulk_cpu_spinbox.pack(anchor=tk.W, pady=(1, 0))
        
        # Memory
        bulk_memory_frame = ttk.Frame(bulk_resource_frame)
        bulk_memory_frame.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(8, 0))
        
        ttk.Label(bulk_memory_frame, text="Memory (GB):", font=("Segoe UI", 8, "bold")).pack(anchor=tk.W)
        bulk_create_memory = tk.StringVar(value="1.0")
        bulk_memory_spinbox = ttk.Spinbox(bulk_memory_frame, from_=0.5, to=8.0, increment=0.5, textvariable=bulk_create_memory, width=6)
        bulk_memory_spinbox.pack(anchor=tk.W, pady=(1, 0))
        
        # Stealth features
        bulk_features_frame = ttk.Frame(bulk_quick_frame)
        bulk_features_frame.pack(fill=tk.X, pady=(3, 0))
        
        bulk_create_disable_webrtc = tk.BooleanVar(value=True)
        ttk.Checkbutton(bulk_features_frame, text="WebRTC", variable=bulk_create_disable_webrtc).pack(side=tk.LEFT, padx=(0, 10))
        
        bulk_create_disable_canvas = tk.BooleanVar(value=True)
        ttk.Checkbutton(bulk_features_frame, text="Canvas", variable=bulk_create_disable_canvas).pack(side=tk.LEFT, padx=(0, 10))
        
        bulk_create_disable_webgl = tk.BooleanVar(value=True)
        ttk.Checkbutton(bulk_features_frame, text="WebGL", variable=bulk_create_disable_webgl).pack(side=tk.LEFT)
        
        
        def create_bulk_profiles():
            print("📝 [BULK-CREATE] Mở dialog tạo hàng loạt")
            source = bulk_source_var.get()
            ...
            try:
                ...
                stealth_config = None
                if bulk_use_stealth_var.get():
                    stealth_config = {
                        'user_agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/139.0.0.0 Safari/537.36',
                        'cpu_cores': int(bulk_create_cpu_cores.get()),
                        'memory_gb': float(bulk_create_memory.get()),
                        'screen_width': 1920,
                        'screen_height': 1080,
                        'disable_webrtc': bulk_create_disable_webrtc.get(),
                        'disable_canvas': bulk_create_disable_canvas.get(),
                        'disable_webgl': bulk_create_disable_webgl.get(),
                        'disable_audio': False,
                        'disable_fonts': True,
                        'language': 'en-US',
                        'timezone': 'UTC'
                ...
                result = messagebox.askyesno("Xác nhận", 
                                           ...
                                           f"\n\nTừ profile gốc: {source}?" +
                                           (f"\n\nVới cấu hình Stealth: CPU {stealth_config['cpu_cores']} cores, Memory {stealth_config['memory_gb']} GB" if stealth_config else ""))
                ...
                def create_bulk_thread():
                    ...
                    for i, name in enumerate(names):
                        try:
                            ...
                            if success:
                                ...
                                if stealth_config:
                                    config_name = f"stealth_{name}"
                                    print(f"📝 [BULK-CREATE] Stealth config được tạo cho {name} (chưa lưu)")
                                    # TODO: Implement save_stealth_config function
                        ...
                        # Update progress
                        progress = f"Đang tạo profile {i+1}/{quantity}..."
                        self.root.after(0, lambda p=progress: self.status_label.config(text=p))
                    
                    # Hiển thị kết quả
                    result_msg = f"✅ Đã tạo thành công: {success_count} profile\n"
                    if error_count > 0:
                    ...
                    self.root.after(0, lambda: messagebox.showinfo("Kết quả", result_msg))
                    self.root.after(0, lambda: self.refresh_profiles())
                    self.root.after(0, lambda: self.status_label.config(text="Hoàn thành tạo profile hàng loạt"))
        ...
        bulk_stealth_options_frame.pack_forget()
    ...
    def launch_profile(self, hidden=None):
        ...
        print(f"🚀 [LAUNCH] Khởi động profile: {profile_name}")
    ...
    def bulk_run_selected(self):
        ...
        print("🚀 [BULK-RUN] Mở dialog chạy hàng loạt")
        selected_profiles = []
        try:
        ...
        url_var = tk.StringVar(value="https://www.tiktok.com/login/phone-or-email/email")
...
# Format: username|password"""
        ...
        def start_bulk_run():
            print("🚀 [BULK-RUN] Bắt đầu chạy hàng loạt")
            ...
            for line in accounts_text_content.split('\n'):
                line = line.strip()
                if not line:
                    continue
                account_data = None
                if format_type == "TikTok Format":
                    # Parse TikTok format: username|password|email|email_password|ms_refresh_token|ms_client_id|[session_token]|[user_id]
        ...
        start_btn = ttk.Button(buttons_frame, text="🚀 Bắt đầu", command=start_bulk_run)
...
Sau khi đăng nhập, profile sẽ được đồng bộ với tài khoản Google của bạn."""
        ...
        def start_chrome_login():
            ...
            def login_thread():
                try:
                    # Khởi động Chrome profile ở chế độ hiển thị
                    success, result = self.manager.launch_chrome_profile(
                        profile_name, 
        ...
        # Nút
        ttk.Button(button_frame, text="🚀 Mở Chrome", command=start_chrome_login).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(button_frame, text="❌ Hủy", command=cancel).pack(side=tk.LEFT)
...
"""
    ...
    def install_extension_selected(self):
        ...
        try:
            ...
            self.extension_status_text.delete(1.0, tk.END)
            self.extension_status_text.insert(tk.END, f"🚀 Installing Proxy SwitchyOmega 3 for {len(selected_profiles)} profiles...\n\n")
            self.extension_status_text.update()
    ...
    def install_extension_for_all_profiles(self):
        ...
        try:
            ...
            self.extension_status_text.delete(1.0, tk.END)
            self.extension_status_text.insert(tk.END, f"🚀 Installing Proxy SwitchyOmega 3 for ALL {len(all_profiles)} profiles...\n\n")
            self.extension_status_text.update()
...
• 500,000+ users, 4.8/5 rating
            ...
            ttk.Checkbutton(options_frame, text="🚀 Install for all profiles", 
            ...
            ttk.Button(button_frame, text="🚀 Start Installation", command=start_installation).pack(side=tk.RIGHT)
        ...
        try:
            def startup_install_thread():
                try:
                    ...
                    self.extension_status_text.delete(1.0, tk.END)
                    self.extension_status_text.insert(tk.END, "🚀 Starting automatic extension installation...\n")
    ...
    def _copy_extensions_between_profiles(self, source_profile, target_profile):
        ...
        try:
            ...
            source_path = os.path.join("chrome_profiles", source_profile)
            target_path = os.path.join("chrome_profiles", target_profile)
    ...
    def show_pac_tab_disabled(self):
        ...
        self.update_tab_highlight('pac_files')
        self.clear_content()
    ...
    def apply_proxy_input(self):
        ...
        try:
            ...
            if success:
                ...
                self.log_proxy_status("🚀 Importing into extension to activate...")
    ...
    def force_import_proxy(self):
        ...
        if not target_profile:
            ...
            return
        try:
            self.log_proxy_status(f"🚀 Importing settings into extension for: {target_profile}")
    ...
    def bulk_apply_from_text(self):
        ...
        try:
            ...
            def run_apply():
                ...
                self.proxy_status_text.insert(tk.END, f"🚀 Bulk apply {count} proxies to {count} profiles\n")
