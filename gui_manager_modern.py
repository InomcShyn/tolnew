            ...
            for i, profile_name in enumerate(profiles):
                ...
                if i < len(accounts):
                    ...
                    if success:
                        ...
                        # ✅ PERFORM AUTOFILL if login_data exists
                        if login_data and login_data.get('username') and login_data.get('password'):
                            print(f"⌨️ [BULK-RUN] Performing autofill for {profile_name}...")
                            try:
                                import asyncio
                                try:
                                ...
                                async def perform_autofill():
                                    """Perform keyboard autofill"""
                                    try:
                                        page = result
                                        print(f"[AUTOFILL] Waiting for page to load...")
                                        ...
                                        for selector in username_selectors:
                                            try:
                                                ...
                                                await page.fill(selector, login_data['username'])
                                                print(f"✅ [AUTOFILL] Filled username with selector: {selector}")
                                        ...
                                        if not username_filled:
                                            print(f"❌ [AUTOFILL] Could not find username field")
                                            return False
                                        ...
                                        for selector in password_selectors:
                                            try:
                                                await page.wait_for_selector(selector, timeout=2000)
                                                await page.fill(selector, login_data['password'])
                                                print(f"✅ [AUTOFILL] Filled password with selector: {selector}")
                                        ...
                                        if not password_filled:
                                            print(f"❌ [AUTOFILL] Could not find password field")
                                            return False
                                        ...
                                        for selector in login_button_selectors:
                                            try:
                                                ...
                                                print(f"✅ [AUTOFILL] Clicked login button with selector: {selector}")
                                                button_clicked = True
                                                break
                                            except:
                                                continue
                                        ...
                                        if not button_clicked:
                                            print(f"❌ [AUTOFILL] Could not find login button")
                                            # Try pressing Enter as fallback
                                            try:
                                                await page.keyboard.press('Enter')
                                                print(f"✅ [AUTOFILL] Pressed Enter as fallback")
                                            except:
                                                return False
                                        
                                        print(f"✅ [AUTOFILL] Autofill completed, waiting for response...")
                                        return True
                                    except Exception as e:
                                ...
                                autofill_success = loop.run_until_complete(perform_autofill())
                                if autofill_success:
                                    print(f"✅ [BULK-RUN] Autofill completed for {profile_name}")
                                    print(f"⏳ [BULK-RUN] Waiting 5 seconds for login to process...")
                                    time.sleep(5)  # Wait longer for login to process
                                else:
                                    print(f"⚠️ [BULK-RUN] Autofill failed for {profile_name}")
                            except Exception as e:
                                print(f"❌ [BULK-RUN] Autofill error: {e}")
                        ...
                        if use_api_directly and omocaptcha_api_key:
                            ...
                            try:
                                ...
                                # Run captcha check and solve
                                captcha_solved = loop.run_until_complete(check_and_solve_captcha_v2())
                                
                                if not captcha_solved:
                                    print(f"⚠️ [CAPTCHA-V2] Captcha not solved, login may fail")
                        ...
                        # ✅ FIX: LUÔN LUÔN thử lấy OTP sau khi autofill (không phụ thuộc vào type_cfg)
                        # Chỉ cần có raw_line (account string) là thử lấy OTP
                                    ...
                                    if result and hasattr(self.manager, '_input_verification_code'):
                                        try:
                                            ...
                                            if auto_otp_done:
                                                ...
                                                # ✅ FIX: Sau khi điền OTP, BẮT BUỘC chờ ít nhất 10s để TikTok xử lý
                                                # Không phụ thuộc vào type_cfg
                                                wait_seconds = max(10, wait_seconds)  # Tối thiểu 10s
                        ...
                        if login_success_detected:
                            ...
                            login_data['login_error'] = login_error_msg or "Login failed - not redirected to foryou"
                            print(f"❌ [BULK-RUN] Account {profile_name} login thất bại → DIE")
                            self.root.after(0, lambda p=profile_name: self._update_profile_status(p, "🔴 Die"))
