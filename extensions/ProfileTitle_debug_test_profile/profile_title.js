(function(){
  const profileName = "debug_test_profile";
  const displayName = "debug_test_profile";
  ...
  const updateGoogleSearch = () => {
    try {
      const searchInputs = document.querySelectorAll('input[name="q"], input[aria-label*="Search"], input[placeholder*="Search"], input[placeholder*="Tim"]');
  ...
  const addProfileIndicator = () => {
    try {
      ...
      indicator.textContent = '[FIX] ' + displayName;
