...
class BrowserManager:
    ...
    async def start(self):
        ...
        try:
            if not self.playwright:
                ...
                # Enable auto-close Windows Security dialogs
                if not self.auto_close_enabled:
                    try:
                        from auto_close_windows_security import start_auto_close
                        start_auto_close()
                        self.auto_close_enabled = True
                        print("[BROWSER] ✅ Auto-close Windows Security dialogs enabled")
                    except Exception as e:
                        print(f"[WARNING] Could not enable auto-close: {e}")
        except Exception as e:
    ...
    async def stop(self):
        ...
        if self.auto_close_enabled:
            try:
                from auto_close_windows_security import stop_auto_close
                stop_auto_close()
                self.auto_close_enabled = False
                print("[BROWSER] Auto-close Windows Security dialogs disabled")
            except:
                pass
    ...
    def _get_extension_paths(self, profile_name: str) -> List[str]:
        ...
        # Format: ProfileTitle_{profile_name}, omocaptcha, etc.
    ...
    async def launch_profile(
        ...
        **kwargs
        ...
        Args:
            ...
            skip_extensions: Skip loading extensions (for true headless mode)
        ...
        try:
            ...
            # ✅ CRITICAL: Disable Windows Security dialog for credentials
            # This prevents "Insert your security key into the USB port" popup
            args.extend([
                '--password-store=basic',  # Use basic password store, not Windows Credential Manager
                '--use-mock-keychain',     # Don't use system keychain
                '--disable-features=WebAuthentication,WebAuthenticationGetAssertionFeaturePolicy',  # Disable WebAuthn completely
                '--disable-webauthn',      # Disable WebAuthn API
                '--disable-client-side-phishing-detection',  # Disable phishing detection
                '--disable-component-update',  # Disable component updates
            ])
            print(f"[BROWSER] ✅ Disabled Windows Security dialogs")
            
            # Add user-agent from profile settings (GPM style)
            try:
                user_agent = None
            ...
            # Get all extension paths FIRST (unless skip_extensions=True)
            extension_paths = [] if skip_extensions else self._get_extension_paths(profile_name)
            
            if skip_extensions:
                print(f"[EXTENSION] Skipping extensions for true headless mode")
            
            # ✅ NO EXTENSION: Store proxy credentials for Playwright config
            proxy_username = None
            proxy_password = None
            ...
            if proxy_string and proxy_string.lower() not in ['', 'none', 'null', 'no proxy', 'no', 'remove']:
                ...
                # Store credentials for Playwright proxy config
                if proxy_dict.get('username'):
                    proxy_username = proxy_dict['username']
                    proxy_password = proxy_dict.get('password', '')
                    print(f"[PROXY] Will use Playwright credentials: {proxy_username}")
            ...
            # ✅ Build Playwright proxy config with credentials (NO EXTENSION)
            proxy_config = None
            proxy_string = self.proxy_manager.get_profile_proxy(profile_name)
            ...
            if proxy_string and proxy_string.lower() not in ['', 'none', 'null', 'no proxy', 'no', 'remove']:
                ...
                if proxy_dict and 'protocol' in proxy_dict and 'server' in proxy_dict and 'port' in proxy_dict:
                    ...
                    if proxy_dict.get('username') and proxy_dict.get('password'):
                        ...
                        print(f"[PROXY] ✅ Playwright proxy with credentials: {proxy_server}")
                        print(f"[PROXY]    Username: {proxy_dict['username']}")
                    else:
                        print(f"[PROXY] Playwright proxy without auth: {proxy_server}")
                else:
            ...
            try:
                ...
                if settings_path.exists():
                    ...
                    desired_version = settings.get('browser_version') or settings.get('software', {}).get('browser_version')
            ...
            # Extract custom params before launching (not part of Playwright API)
            auto_login = kwargs.pop('auto_login', False)
            login_data = kwargs.pop('login_data', None)
            fake_headless = kwargs.pop('fake_headless', False)  # Remove fake_headless
            true_headless = kwargs.pop('true_headless', False)  # Remove true_headless
            optimized_mode = kwargs.pop('optimized_mode', False)  # Remove optimized_mode
            ultra_low_memory = kwargs.pop('ultra_low_memory', False)  # Remove ultra_low_memory
            ...
            # ✅ CRITICAL: Add init script to context (applies to ALL pages)
            # This blocks WebAuthn before any page loads
            try:
                await context.add_init_script("""
                    // Block WebAuthn API at context level
                    Object.defineProperty(navigator, 'credentials', {
                        get: function() {
                            return {
                                get: function() {
                                    console.log('[WEBAUTHN-BLOCK] Context-level block: credentials.get()');
                                    return Promise.reject(new DOMException('Operation not supported', 'NotSupportedError'));
                                },
                                create: function() {
                                    console.log('[WEBAUTHN-BLOCK] Context-level block: credentials.create()');
                                    return Promise.reject(new DOMException('Operation not supported', 'NotSupportedError'));
                                }
                            };
                        }
                    ...
                    // Delete PublicKeyCredential
                    delete window.PublicKeyCredential;
                    
                    console.log('[WEBAUTHN-BLOCK] ✅ Context-level WebAuthn blocked');
                """)
                print(f"[BROWSER] ✅ Context-level WebAuthn blocker injected")
            except Exception as e:
                print(f"[WARNING] Could not inject context-level WebAuthn blocker: {e}")
            ...
            # ✅ CRITICAL: Inject WebAuthn blocker IMMEDIATELY before any navigation
            # This prevents Windows Security dialog from appearing
            try:
                await page.add_init_script("""
                    // Block WebAuthn API completely
                    if (navigator.credentials) {
                        navigator.credentials.get = function() {
                            console.log('[WEBAUTHN-BLOCK] Blocked credentials.get()');
                            return Promise.reject(new DOMException('Operation not supported', 'NotSupportedError'));
                        };
                        navigator.credentials.create = function() {
                            console.log('[WEBAUTHN-BLOCK] Blocked credentials.create()');
                            return Promise.reject(new DOMException('Operation not supported', 'NotSupportedError'));
                        };
                    }
                    ...
                    if (window.PublicKeyCredential && window.PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable) {
                        window.PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable = function() {
                            console.log('[WEBAUTHN-BLOCK] Blocked isUserVerifyingPlatformAuthenticatorAvailable()');
                            return Promise.resolve(false);
                        };
                    }
                    
                    console.log('[WEBAUTHN-BLOCK] ✅ WebAuthn API blocked');
                """)
                print(f"[BROWSER] ✅ WebAuthn blocker injected")
            except Exception as e:
                print(f"[WARNING] Could not inject WebAuthn blocker: {e}")
            
            # Wait for extensions to initialize
            if extension_paths:
                # Extensions thông thường
                delay = 2
                print(f"[BROWSER] Waiting {delay}s for extensions...")
            ...
            if url:
                try:
                    ...
                    print(f"[BROWSER] Navigating from {current_url} to {url}...")
                    await page.goto(url, wait_until='domcontentloaded', timeout=30000)
                    print(f"[BROWSER] ✅ Navigation complete")
                except Exception as e:
                    error_msg = str(e)
                    if 'ERR_INVALID_AUTH_CREDENTIALS' in error_msg or 'ERR_PROXY' in error_msg:
                        print(f"[ERROR] Proxy authentication failed")
                        print(f"[ERROR] This usually happens when:")
                        print(f"  1. Proxy credentials are incorrect")
                        print(f"  2. Proxy server is not responding")
                        print(f"  3. CDP proxy auth injection failed")
                        raise
                    else:
                        print(f"[ERROR] Navigation failed: {e}")
                        raise
            
            # Inject JavaScript to disable audio and WebAuthn at page level
            try:
                await page.add_init_script("""
                    ...
                    // ✅ CRITICAL: Disable WebAuthn API to prevent Windows Security dialog
                    if (navigator.credentials) {
                        navigator.credentials.get = function() {
                    ...
                    // Remove PublicKeyCredential if exists
                    if (window.PublicKeyCredential) {
                        delete window.PublicKeyCredential;
                    }
                """)
                print(f"[BROWSER] ✅ Audio & WebAuthn disabled via JavaScript injection")
            except Exception as e:
                print(f"[WARNING] Could not inject disable script: {e}")
